...
function add(funcid){
	...
	$.post("operButton_add.do",data,saveComplete);
}
function saveComplete(data){
	...
	if(startsWith(data,"1")){
		...
		if(attr!=null && attr.length>1){
		   ...
		   +"   </td>"
		   +"   <td >"
		   +" <input type=\"button\" value=\"修改\" onclick=\"update('"+attr[1]+"')\" class=\"button\">"
		   +" <input type=\"button\" value=\"删除\" onclick=\"del('"+attr[1]+"')\" class=\"button\">"
		  +"    </td>"
		  +"  </tr>";
		  $("#rows").append(str);
		}
...
function update(id){
	...
	$.post("operButton_update.do",data,updateComplete);
}
...
function del(id){
	if(confirm("确定删除操作按钮么?")){
		$("#state").fadeIn("slow"); 
		var data="pageContentNoFilter=1&operbutton.id="+id+"&operbutton.name="+$.trim(replaceAll($("#name"+id).val(),"&","<参数>"));
		$.post("operButton_del.do",data,delComplete);
	}
}
