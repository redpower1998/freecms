	...
	public String editDo(){
		if (info!=null) {
			...
			try {
				...
				if (listPageFuncId!=null && listPageFuncId.trim().length()>0) {
					write("<script>alert('操作成功');location.href='info_list.do?info.channel="+info.getChannel()+"&pageFuncId="+listPageFuncId+"';</script>", "GBK");
				}else {
					write("<script>alert('操作成功');location.href='info_edit.do?info.id="+info.getId()+"&pageFuncId="+pageFuncId+"';</script>", "GBK");
				}
			} catch (Exception e) {
				DBProException(e);
